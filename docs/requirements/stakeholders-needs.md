...
| **ОСНОВНИЙ СЦЕНАРІЙ:** | 1. Гість починає взаємодію з системою. <br/> 2. Гість переходить на сторінку пошуку. <br/> 3. Гість вводить дані в поле пошуку. <br/> 4. Гість натискає кнопку "Пошук". <br/> 5. Система проводить пошук у базі даних (можлива SearchError). <br/> 6. Система відображає результати пошуку гостю. <br/>|
...
| **ВИКЛЮЧНІ СИТУАЦІЇ:** | EmptyRequiredSignInField - Обов'язкові поля для входу не заповнені. <br/>IncorrectSignInPassword - Невірний пароль введений користувачем. <br/>NotFoundSignInUser - Користувач з вказаною електронною поштою не знайдений. |
| **ОСНОВНИЙ СЦЕНАРІЙ:** | 1. Гість обирає "Увійти в систему" у меню вебсайту. <br/>2. Система автоматично перенаправляє гостя на сторінку з формою входу. <br/>3. Гість заповнює форму входу.<br/> 4. Після заповнення форми, гість натискає кнопку "Увійти", щоб відправити дані для обробки (можлива EmptyRequiredSignInField).<br/>5. Система перевіряє, чи існує обліковий запис з вказаними даними (можлива NotFoundSignInUser).<br/>6. Система перевіряє введений гостем пароль (можлива IncorrectSignInPassword).<br/>7. Якщо дані введені вірно, система надає доступ до облікового запису користувача.<br/> 8. Гість отримує повідомлення про успішний вхід у систему.<br/>9. Система автоматично перенаправляє користувача на головну сторінку. |
...
| **УЧАСНИКИ:**          | Користувач, Система. |
| **ПЕРЕДУМОВИ:**        | Користувач увійшов до системи. |
| **РЕЗУЛЬТАТ:**         | Інформація відповідно до запиту представлена нижче. |
| **ВИКЛЮЧНІ СИТУАЦІЇ:** | Дані відсутні - SearchError. |
| **ОСНОВНИЙ СЦЕНАРІЙ:** | 1. Користувач активує опцію "Пошук" на головній сторінці веб-сайту.<br/>2. Вводить необхідні дані в поле пошуку.<br/>3. Користувач натискає кнопку "Знайти".<br/>4. Система перевіряє доступні дані в базі даних (можлива помилка - SearchError).<br/>5. Результати пошуку відображаються на екрані. |
...
| **ВИКЛЮЧНІ СИТУАЦІЇ:** | Швидкість завантаження та якість з'єднання не дозволяють завантажити файл - BadConnection. <br/>Файл з даними у відповідному форматі не доступний у системі - DataFormatNotAvailable. |
| **ОСНОВНИЙ СЦЕНАРІЙ:** | 1. Користувач обирає опцію "Завантажити на пристрій" на знайденій сторінці веб-сайту.<br/>2. Вибирає формат файлу для завантаження.<br/>3. Система перевіряє наявність відповідного формату файлу у базі даних (можлива помилка - DataFormatNotAvailable).<br/> 4. Система розпочинає передачу файлу у вибраному форматі на комп'ютер користувача (можлива помилка - BadConnection).<br/>5. Після успішного завершення завантаження, система повідомляє про це користувача. |
...
| **ВИКЛЮЧНІ СИТУАЦІЇ:** | Жодних. |
| **ОСНОВНИЙ СЦЕНАРІЙ:** | 1. Адміністратор обирає опцію "Вийти" на головній сторінці веб-сайту.<br/> 2. Система відкриває вікно підтвердження виходу. <br/> 3. Адміністратор підтверджує свій намір вийти, натискаючи "Так". <br/> 4. Система завершує сеанс адміністратора. <br/> 5. Після виходу, система автоматично перенаправляє адміністратора на головну сторінку. |
...
| **ВИКЛЮЧНІ СИТУАЦІЇ:** | Жодних. |
| **ОСНОВНИЙ СЦЕНАРІЙ:** | 1. Адміністратор переходить на сторінку, де можна переглянути запити.<br/> 2. Він натискає кнопку "Одобрити запит".<br/> 3. Система надає доступ до нових даних користувачам.<br/> 4. Після цього система автоматично видаляє запит зі списку. |

| **ID:**                | `Admin.DeniedRequest`                                                                             |
|------------------------|--------------------------------------------------------------------------------------------------|
